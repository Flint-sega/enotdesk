...
## Current state
Working MVP после волны правок по код-ревью: Node server (control plane + WS signaling) и Electron-клиент с реальным нативным вводом на macOS; Win/X11 адаптеры, видео между машинами и Win/Linux-сборки по-прежнему не проверены на живых машинах. Новое: клавиатура по физическим кодам (любая раскладка), скролл в строках, грейс-период переподключения 30с (ADR 0013), в сессии — чат, буфер обмена и передача файлов поверх DataChannel (ADR 0014), видео-UX (fullscreen, fit, 2,5 Мбит/с, смена экрана replaceTrack), ретенция БД, защитные заголовки страниц, HEAD у файлов сборок, /audit только admin+auditor, потолок waiting-сессий на IP, CI (GitHub Actions: npm test + smoke). Репозиторий: `client/`, `server/`, `build/`, `assets/`, `scripts/`, `docs/`, `archive/mockup/` — старый макет, не приложение.

## Commands
- `npm install` — deps (node>=24.12; koffi, ws exact-pinned; electron, electron-builder dev).
- `npm test` — node:test, 115 tests, server+client, no Electron needed (проверено фактически: 115/115 pass).
...
## Structure
```
server/          control plane: app.mjs (HTTP /api/v1 + WS /signal + файлы), pages.mjs (страницы /downloads, /invite), db.mjs (node:sqlite, ретенция), crypto.mjs (scrypt, tokens), bootstrap.mjs, test/
client/          Electron: main.mjs (window, tokens, WS, input wiring), preload.cjs (window.enot bridge)
client/lib/      Electron-free shims: api.mjs, signal.mjs, protocol.mjs, input-pipeline.mjs, native-input.mjs, keymap.mjs (e.code→ключ), chat.mjs, clipboard-sync.mjs, file-transfer.mjs, credentials.mjs, invites.mjs, server-url.mjs
...
.github/         workflows/test.yml — CI: npm ci + npm test + smoke:local на node 24
...
## Environment
`ENOT_HOST`, `ENOT_PORT`, `ENOT_DB`, `ENOT_DIST_DIR`, `ENOT_PUBLIC_URL`, `ENOT_TURN_URLS`, `ENOT_TURN_USERNAME`, `ENOT_TURN_PASSWORD`, `ENOT_GRACE_MS`, `ENOT_RETENTION_DAYS` (see `.env.example`) — never commit values; деплой также читает `ENOT_DEPLOY_HOST/USER/PORT/PASSWORD/PROTO` и `ENOT_ADMIN_LOGIN/PASSWORD`; desktop also reads `EDESK_SMOKE=1`.

## Tests
`node --test server/test/*.test.mjs client/test/*.test.mjs` — 115 тестов (server HTTP/WS + client lib-шимы + renderer-contract + ретенция), фактически зелёные; single file: `node --test server/test/lifecycle.test.mjs`. `smoke:local` covers the whole control-plane + signaling cycle on a temp DB in ephemeral port. `node scripts/smoke-remote.mjs` — внешний цикл (HTTP+WS) против развёрнутого сервера, деактивирует временного оператора. NOT covered: real input injection into OS (скролл/клавиатура на живых машинах — ручная приёмка), WebRTC video/chat/files across machines, Win/X11 adapters (inert only), Electron UI beyond screenshot smoke — report those honestly, don't fake.

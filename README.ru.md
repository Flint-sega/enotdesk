...
Тесты: `npm test` (135 тестов: серверные HTTP/WS + client-швы + контракт рендерера) и `npm run lint`. Инъекция ввода в ОС тестами не выполняется — проверяются загрузка адаптера, валидация и координаты; реальную инъекцию проверяет человек (см. AGENTS.md).

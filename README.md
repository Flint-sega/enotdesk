...
- **Клиент (получает помощь):** запускает portable-программу → «Получить помощь» → видит ID и одноразовый пароль → сообщает их оператору в своём чате → подтверждает подключение конкретного оператора → закрывает окно, когда помощь не нужна. Доступ существует только пока приложение открыто.
- **Оператор (оказывает помощь):** входит на сервере EnotDesk → берёт ID/пароль из чата → подключается → после согласия клиента видит экран и управляет вводом. История сеансов, адресная книга, приглашения и аудит доступны из интерфейса оператора.
- **Транспорт:** видео — WebRTC (DTLS/SRTP) напрямую между сторонами; сигналинг — через ваш сервер EnotDesk; команды ввода — по DataChannel с жёстким allowlist (нормализованные координаты, ограниченный набор клавиш, лимит частоты).
...
Дальше в приложении: вход оператора (или «Получить помощь» на стороне клиента) → адрес сервера по умолчанию `http://127.0.0.1:8080`. Полный связный прогон серверного цикла (bootstrap → вход → приглашение → сессия → claim → согласие → сигналинг): `npm run smoke:local`.

Тесты: `npm test` (46 тестов: серверные HTTP/WS + desktop-швы). Инъекция ввода в ОС тестами не выполняется — проверяются загрузка адаптера, валидация и координаты; реальную инъекцию проверяет человек.
...
Сборка portable-клиента под macOS (проверено на arm64), Windows и Linux: команды, артефакты, системные зависимости, разрешения macOS, подпись — [`docs/BUILD.md`](docs/BUILD.md).
